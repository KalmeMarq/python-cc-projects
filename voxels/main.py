...
class Chunk:
  def __init__(self, x: int, z: int):
    self.x = x
    self.z = z
    self.blocks = np.zeros(16 * 16 * 16, dtype=np.uint8)
...
class World:
  ...
  def set_tile(self, x: int, y: int, z: int, tile_id: int):
    cx = x // 16
    cz = z // 16
    if cx < 0 or cx >= self.x_chunks or cz < 0 or cz >= self.z_chunks or y < 0 or y >= 16:
      return
  ...
  def get_tile(self, x: int, y: int, z: int):
    ...
    if cx < 0 or cx >= self.x_chunks or cz < 0 or cz >= self.z_chunks or y < 0 or y >= 16:
      return 0
    return self.chunks[cz * 2 + cx].get_tile(x % 16, y % 16, z % 16)
